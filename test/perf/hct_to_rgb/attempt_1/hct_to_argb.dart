// Attempt 1: Optimized HCT → ARGB solver for sRGB.
//
// Optimizations vs baseline:
//   1. All pow(x,p).toDouble() → exp(p*log(x)) — avoids num boxing
//   2. Precomputed viewing condition & gamut constants in flat struct
//   3. Flattened matrix access (no List<List<double>> indirection)
//   4. Inlined _inverseChromaticAdaptation (avoid function call + signum)
//   5. Inlined argbFromLinrgbComponents/delinear/delinearized
//   6. max(0,x) → ternary (avoids _Double.compareTo dispatch)
...
/// Inverse chromatic adaptation, inlined.
/// adapted → scaled discount cone response.
/// Uses exp/log instead of pow to stay in double-land.
double _inverseCA(double adapted) {
  final adaptedAbs = adapted < 0.0 ? -adapted : adapted;
  final denom = 400.0 - adaptedAbs;
  if (denom <= 0.0) return 0.0;
  final base = 27.13 * adaptedAbs / denom;
  if (base <= 0.0) return 0.0;
  final result = math.exp(_inv0_42 * math.log(base));
...
(double, double, double)? _findResultByJ(
  ...
  for (var i = 0; i < 5; i++) {
    ...
    // Inline inverse chromatic adaptation × 3
    final rCS = _inverseCA(rA);
    final gCS = _inverseCA(gA);
    final bCS = _inverseCA(bA);
