// Attempt 4: Everything from attempt 3, plus:
//   10. Precomputed Y coefficients (yR, yG, yB) for direct fnj from scaled-discount
//   11. Defer matrix multiply to convergence iteration only
//   Saves 9 mul + 6 add per non-converged iteration (×4 = 60 ops per solve)
// NOTE: Slower than attempt 3.
...
class _SolverConst {
  ...
  // Y directly from scaled-discount (fused k·M)
  final double yR, yG, yB;
  ...
  _SolverConst._({
    ...
    required this.yR, required this.yG, required this.yB,
  ...
  factory _SolverConst._compute() {
    ...
    return _SolverConst._(
      ...
      // yR = kR*m00 + kG*m10 + kB*m20, etc.
      yR: g.yFromLinrgb[0] * m[0][0] + g.yFromLinrgb[1] * m[1][0] + g.yFromLinrgb[2] * m[2][0],
      yG: g.yFromLinrgb[0] * m[0][1] + g.yFromLinrgb[1] * m[1][1] + g.yFromLinrgb[2] * m[2][1],
      yB: g.yFromLinrgb[0] * m[0][2] + g.yFromLinrgb[1] * m[1][2] + g.yFromLinrgb[2] * m[2][2],
...
(double, double, double)? _findResultByJ(
  ...
  final logCT = math.log(chroma * sc.tInnerCoeff);
  ...
  for (var i = 0; i < 5; i++) {
    ...
    final logJN = math.log(jN);
    final ac = sc.aw * math.exp(sc.invCZ * logJN);
    // t = (alpha * tIC)^(1/0.9) where alpha = chroma / sqrt(jN)
    //   = (chroma * tIC)^(1/0.9) * jN^(-0.5/0.9)
    //   = exp(inv0_9 * log(chroma*tIC) + tJNCoeff * log(jN))
    final t = math.exp(_inv0_9 * logCT + tJNCoeff * logJN);
    final p2 = ac / sc.nbb;
    final gamma = 23.0 *
        (p2 + 0.305) *
        t /
    ...
    // Compute Y directly from scaled-discount (skip matrix multiply)
    final fnj = sc.yR * rCS + sc.yG * gCS + sc.yB * bCS;
    if (fnj <= 0.0) return null;

    if (i == 4 || (fnj - y).abs() < 0.002) {
      // Converged — now compute linRGB for gamut check + return
      ...
      if (linR < -0.01 || linG < -0.01 || linB < -0.01) return null;
