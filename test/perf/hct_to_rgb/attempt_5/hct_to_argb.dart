// Attempt 5: Everything from attempt 3, plus:
//   10. Fast transcendentals: replace exp()/log() with IEEE-754 bit
//       manipulation + polynomial approximation.
//       fastLog: frexp via bit extract, u=(m-1)/(m+1) series (4 terms, ~15 bits)
//       fastExp: range reduce to [0,ln2), degree-5 Taylor, ldexp via bit set
//       Eliminates ~50% of solver time (exp+log were the wall)
...
final _f64 = Float64List(1);
final _i64 = _f64.buffer.asInt64List();

const _ln2 = 0.6931471805599453;
const _invLn2 = 1.4426950408889634;

/// Fast natural log. ~15 bits accuracy for x > 0.
double _fastLog(double x) {
  _f64[0] = x;
  final bits = _i64[0];
  final e = ((bits >> 52) & 0x7FF) - 1023;
  // Set exponent to 1023 → m in [1, 2)
  _i64[0] = (bits & 0x000FFFFFFFFFFFFF) | 0x3FF0000000000000;
  final m = _f64[0];
  // u = (m-1)/(m+1), |u| < 1/3 → series converges fast
  final u = (m - 1.0) / (m + 1.0);
  final u2 = u * u;
  // ln(m) = 2*u*(1 + u²/3 + u⁴/5 + u⁶/7)  — 4 terms of odd series
  final logM = 2.0 * u * (1.0 + u2 * (0.3333333333333333 +
      u2 * (0.2 + u2 * 0.14285714285714285)));
  return logM + e * _ln2;
}

/// Fast exp. ~14 bits accuracy.
double _fastExp(double x) {
  final nf = x * _invLn2;
  final n = nf.floor();
  final f = x - n * _ln2;
  // Degree-5 Taylor on [0, ln2): exp(f) ≈ 1+f+f²/2+f³/6+f⁴/24+f⁵/120
  final expF = 1.0 + f * (1.0 + f * (0.5 + f * (0.16666666666666666 +
...
(double, double, double)? _findResultByJ(
  ...
  final logCT = _fastLog(chroma * sc.tInnerCoeff);
  // Precompute combined exponent for t: inv0_9 * (-0.5) = -5/9
  const tJNCoeff = _inv0_9 * -0.5; // how log(jN) contributes to t

  for (var i = 0; i < 5; i++) {
    final jN = j * 0.01;
    // Share log(jN) between ac and t
    final logJN = _fastLog(jN);
    final ac = sc.aw * _fastExp(sc.invCZ * logJN);
    // t = (chroma * tIC)^(1/0.9) * jN^(-0.5/0.9)
    final t = _fastExp(_inv0_9 * logCT + tJNCoeff * logJN);
    final p2 = ac / sc.nbb;
    final gamma = 23.0 *
        (p2 + 0.305) *
        t /
    ...
    // Inverse chromatic adaptation — fast transcendentals
    double rCS, gCS, bCS;
    {
      ...
      if (d1 <= 0.0) { rCS = 0.0; } else {
        ...
        rCS = b1 <= 0.0 ? 0.0 : _fastExp(_inv0_42 * _fastLog(b1));
        if (rA < 0.0) rCS = -rCS;
      }
    }
    {
      ...
      if (d1 <= 0.0) { gCS = 0.0; } else {
        final b1 = 27.13 * a1 / d1;
        gCS = b1 <= 0.0 ? 0.0 : _fastExp(_inv0_42 * _fastLog(b1));
        if (gA < 0.0) gCS = -gCS;
      }
    }
    {
      ...
      if (d1 <= 0.0) { bCS = 0.0; } else {
        final b1 = 27.13 * a1 / d1;
        bCS = b1 <= 0.0 ? 0.0 : _fastExp(_inv0_42 * _fastLog(b1));
        if (bA < 0.0) bCS = -bCS;
      }
    }
