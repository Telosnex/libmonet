...
class QuantizerWsmeansOpt {
  ...
  static QuantizerResult quantize(
    ...
    for (var i = 0; i < clusterCount; i++) {
      ...
      if (seenClusters.contains(possibleNewCluster)) {
        continue;
      }

      seenClusters.add(possibleNewCluster);
    ...
    if (returnInputPixelToClusterPixel) {
      // Precompute ARGB value for each centroid once instead of per-pixel.
      final centroidArgb = List<int>.generate(
          clusters.length, (i) => pointProvider.toInt(clusters[i]));

      final stopwatch = Stopwatch()..start();
      for (var i = 0; i < pixels.length; i++) {
        final inputPixel = pixels[i];
        final clusterIndex = clusterIndices[i];
        final clusterPixel = centroidArgb[clusterIndex];
